...
import { SIP_SEO_DATA } from "@/lib/seo-data";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = SIP_SEO_DATA.find((p) => p.slug === slug);
  if (!page) return {};
  ...
  return {
    ...
    alternates: {
      canonical: `/sip-calculator/${page.slug}`,
    },
  };
}
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-10">
      <nav className="flex items-center gap-2 text-sm text-slate-500">
        ...
        <Link href="/sip-calculator" className="hover:text-slate-900">SIP Calculator</Link>
        <ChevronRight className="w-3 h-3" />
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <section className="section-panel p-5 sm:p-6">
          <h2 className="text-2xl font-bold mb-4">Understanding the {page.h1}</h2>
        ...
        <section className="section-panel p-5 sm:p-6">
          <h2 className="text-2xl font-bold mb-4">SIP FAQs</h2>
      ...
      <section className="section-panel p-5 sm:p-6">
        <h2 className="text-2xl font-bold mb-8">Related Financial Tools</h2>
        <div className="grid md:grid-cols-2 gap-6">
          <Link href="/income-tax-calculator" className="flex items-center justify-between rounded-lg border border-slate-200 bg-white p-4 text-sm font-semibold capitalize text-slate-700 transition-all hover:border-slate-400 hover:shadow-sm">
            <div>
              <h3 className="font-bold text-lg mb-1">Income Tax Calculator</h3>
              <p className="text-slate-500 text-xs font-normal">Compare Old vs New Tax Regimes</p>
            </div>
          ...
          <Link href="/emi-calculator" className="flex items-center justify-between rounded-lg border border-slate-200 bg-white p-4 text-sm font-semibold capitalize text-slate-700 transition-all hover:border-slate-400 hover:shadow-sm">
            <div>
              ...
              <p className="text-slate-500 text-xs font-normal">Plan your loans effectively</p>
            </div>
            <ChevronRight className="w-5 h-5 text-slate-400" />
          </Link>
        </div>
